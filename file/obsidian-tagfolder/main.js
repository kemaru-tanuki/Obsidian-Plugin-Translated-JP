...
var DEV = false,
  ...
  DESTROYED = 8192,
  EFFECT_RAN = 16384,
  EFFECT_TRANSPARENT = 32768,
  LEGACY_DERIVED_PROP = 65536,
  INSPECT_EFFECT = 1 << 17,
  HEAD_EFFECT = 1 << 18,
...
function hydration_attribute_changed(attribute, html2, value) {
  ...
  else console.warn("hydration_attribute_changed");
}
...
function hydration_html_changed(location) {
  ...
  else console.warn("hydration_html_changed");
}
...
function hydration_mismatch(location) {
  ...
  else console.warn("hydration_mismatch");
}
...
function lifecycle_double_unmount() {
  ...
  else console.warn("lifecycle_double_unmount");
}
...
function ownership_invalid_binding(parent, child2, owner) {
  ...
  else console.warn("ownership_invalid_binding");
}
  ...
  if (DEV)
    console.warn(
      "%c[svelte] ownership_invalid_mutation\n%c" +
        (component2
          ? `${component2} mutated a value owned by ${owner}. This is strongly discouraged. Consider passing values to child components with \`bind:\`, or use a callback instead`
          : "Mutating a value outside the component that created it is strongly discouraged. Consider passing values to child components with `bind:`, or use a callback instead"),
      bold,
      normal
    );
  else console.warn("ownership_invalid_mutation");
}
...
function state_proxy_equality_mismatch(operator) {
  ...
  else console.warn("state_proxy_equality_mismatch");
}
...
function get_stack() {
  ...
  for (const line of stack2.split("\n")) {
    let match =
      null != (_a = chrome_pattern.exec(line))
        ? _a
        : firefox_pattern.exec(line);
    if (match)
      entries.push({
        file: match[1],
        line: +match[2],
...
function add_owner(object, owner, global = false, skip_warning = false) {
  if (object && !global) {
    const component2 = dev_current_component_function,
      metadata = object[STATE_SYMBOL_METADATA];
...
function add_owner_to_object(object, owner, seen) {
  ...
  if (metadata) {
    if ("owners" in metadata && null != metadata.owners)
    ...
    if (ADD_OWNER in object && object[ADD_OWNER])
      ...
      if (proto === Object.prototype)
        for (const key in object) add_owner_to_object(object[key], owner, seen);
...
function has_owner(metadata, component2) {
  if (null === metadata.owners) return true;
  else
    return (
      metadata.owners.has(component2) ||
      (null !== metadata.parent && has_owner(metadata.parent, component2))
    );
}
...
function get_owner(metadata) {
  ...
  return null !=
    (_b =
      null == (_a = null == metadata ? void 0 : metadata.owners)
        ? void 0
...
function check_ownership(metadata) {
  ...
  if (component2 && !has_owner(metadata, component2)) {
    ...
    else ownership_invalid_mutation();
  }
}
...
function safe_not_equal(a, b) {
  return a != a
    ? b == b
    : a !== b || (null !== a && "object" == typeof a) || "function" == typeof a;
}
...
function mutable_source(initial_value, immutable = false) {
  ...
  if (null !== component_context && null !== component_context.l)
    (null != (_b = (_a = component_context.l).s) ? _b : (_a.s = [])).push(s);
  return s;
}

function push_derived_source(source2) {
  if (null !== active_reaction && !!(active_reaction.f & DERIVED))
    if (null === derived_sources) set_derived_sources([source2]);
    else derived_sources.push(source2);
  return source2;
}
...
function set(source2, value) {
  if (
    ...
    is_runes() &&
    !!(active_reaction.f & (DERIVED | BLOCK_EFFECT)) &&
    (null === derived_sources || !derived_sources.includes(source2))
  )
    state_unsafe_mutation();
...
function internal_set(source2, value) {
  if (!source2.equals(value)) {
    ...
    if (
      is_runes() &&
      null !== active_effect &&
      !!(active_effect.f & CLEAN) &&
      !(active_effect.f & BRANCH_EFFECT)
    )
      if (null !== new_deps && new_deps.includes(source2)) {
        set_signal_status(active_effect, DIRTY);
        schedule_effect(active_effect);
      } else if (null === untracked_writes) set_untracked_writes([source2]);
    ...
    for (
      ...
      if (!(flags & DIRTY))
        if (runes || reaction !== active_effect)
          if (!(DEV && flags & INSPECT_EFFECT)) {
            set_signal_status(reaction, status);
            if (!!(flags & (CLEAN | UNOWNED)))
              if (!!(flags & DERIVED)) mark_reactions(reaction, MAYBE_DIRTY);
              else schedule_effect(reaction);
...
function derived(fn) {
  var _a,
    flags = DERIVED | DIRTY;
  if (null === active_effect) flags |= UNOWNED;
  else active_effect.f |= EFFECT_HAS_DERIVED;
  const signal = {
    children: null,
  ...
  if (null !== active_reaction && !!(active_reaction.f & DERIVED)) {
    ...
    (null != (_a = derived3.children) ? _a : (derived3.children = [])).push(
      signal
    );
  }
  return signal;
...
function destroy_derived_children(derived3) {
  ...
  if (null !== children) {
    ...
    for (var i = 0; i < children.length; i += 1) {
      ...
      if (!!(child2.f & DERIVED)) destroy_derived(child2);
      else destroy_effect(child2);
    }
  }
}
...
function update_derived(derived3) {
  ...
  set_signal_status(
    derived3,
    (skip_reaction || !!(derived3.f & UNOWNED)) && null !== derived3.deps
      ? MAYBE_DIRTY
      : CLEAN
  );
...
function check_dirtiness(reaction) {
  ...
  if (!!(flags & MAYBE_DIRTY)) {
    var dependencies = reaction.deps,
      is_unowned = !!(flags & UNOWNED);
    if (null !== dependencies) {
      var i;
      if (!!(flags & DISCONNECTED)) {
        for (i = 0; i < dependencies.length; i++)
          (null != (_b = (_a = dependencies[i]).reactions)
            ? _b
            : (_a.reactions = [])
          ).push(reaction);
      ...
      for (i = 0; i < dependencies.length; i++) {
        ...
        if (
          ...
          null !== active_effect &&
          !skip_reaction &&
          !(null == (_c = null == dependency ? void 0 : dependency.reactions)
            ? void 0
            : _c.includes(reaction))
        )
          (null != (_d = dependency.reactions)
            ? _d
...
function handle_error(error, effect2, component_context2) {
  var _a, _b;
  if (!DEV || handled_errors.has(error) || null === component_context2)
    throw error;
  const component_stack = [],
    effect_name = null == (_a = effect2.fn) ? void 0 : _a.name;
  if (effect_name) component_stack.push(effect_name);
  let current_context = component_context2;
  for (; null !== current_context; ) {
    if (DEV) {
      var filename =
        null == (_b = current_context.function) ? void 0 : _b[FILENAME];
      if (filename) {
...
function update_reaction(reaction) {
  var _a,
    _b,
    previous_deps = new_deps,
    previous_skipped_deps = skipped_deps,
    previous_untracked_writes = untracked_writes,
    previous_reaction = active_reaction,
    previous_skip_reaction = skip_reaction,
    prev_derived_sources = derived_sources,
...
function remove_reaction(signal, dependency) {
  ...
  if (null !== reactions) {
    ...
    if (-1 !== index2) {
      ...
      if (0 === new_length) reactions = dependency.reactions = null;
      else {
  ...
  if (
    !(
      null !== reactions ||
      !(dependency.f & DERIVED) ||
      (null !== new_deps && new_deps.includes(dependency))
    )
  ) {
    set_signal_status(dependency, MAYBE_DIRTY);
    if (!(dependency.f & (UNOWNED | DISCONNECTED)))
...
function update_effect(effect2) {
  ...
  if (!(flags & DESTROYED)) {
    ...
    try {
      ...
      if (!!(flags & BLOCK_EFFECT)) destroy_block_effect_children(effect2);
      else destroy_effect_children(effect2);
    ...
    for (var i = 0; i < length; i++) {
      ...
      if (!(effect2.f & (DESTROYED | INERT)) && check_dirtiness(effect2)) {
        ...
        if (
          null === effect2.deps &&
          null === effect2.first &&
          null === effect2.nodes_start
        )
          if (null === effect2.teardown) unlink_effect(effect2);
          else effect2.fn = null;
      }
    }
}
  ...
  main_loop: for (; null !== current_effect; ) {
    ...
    if (!((is_branch && !!(flags & CLEAN)) || flags & INERT))
      if (!!(flags & RENDER_EFFECT)) {
        if (is_branch) current_effect.f ^= CLEAN;
        else if (check_dirtiness(current_effect)) update_effect(current_effect);
...
function flush_sync(fn) {
  var previous_scheduler_mode = scheduler_mode,
    previous_queued_root_effects = queued_root_effects;
  try {
    infinite_loop_guard();
    const root_effects = [];
...
function get(signal) {
  ...
  if (null !== active_reaction) {
    if (null !== derived_sources && derived_sources.includes(signal))
      state_unsafe_local_read();
    var deps = active_reaction.deps;
    if (null === new_deps && null !== deps && deps[skipped_deps] === signal)
      skipped_deps++;
    else if (null === new_deps) new_deps = [signal];
    ...
    var derived3 = signal,
      parent = derived3.parent;
    if (
      null !== parent &&
      !(null == (_a = parent.deriveds) ? void 0 : _a.includes(derived3))
    )
      (null != (_b = parent.deriveds) ? _b : (parent.deriveds = [])).push(
        derived3
      );
  }
  if (is_derived)
    if (check_dirtiness((derived3 = signal))) update_derived(derived3);
  return signal.v;
}
...
function get_or_init_context_map(name) {
  ...
  return null != (_a = component_context.c)
    ? _a
  ...
  if (null !== context_stack_item) {
    ...
    if (DEV)
      dev_current_component_function =
        null !=
        (_b = null == (_a = context_stack_item.p) ? void 0 : _a.function)
          ? _b
          : null;
...
function proxy(value, parent = null, prev) {
  var _a, _b;
  if ("object" != typeof value || null === value || STATE_SYMBOL in value)
    return value;
  ...
  if (DEV) {
    ...
    if (prev) {
      const prev_owners =
        null ==
        (_b = null == (_a = prev.v) ? void 0 : _a[STATE_SYMBOL_METADATA])
          ? void 0
          : _b.owners;
  ...
  return new Proxy(value, {
    ...
    get(target, prop2, receiver) {
      ...
      if (
        void 0 === s &&
        (!exists ||
          (null == (_a2 = get_descriptor(target, prop2))
            ? void 0
            : _a2.writable))
      ) {
      ...
      if (void 0 !== s) {
        ...
        if (DEV) {
          ...
          if (
            prop_metadata &&
            (null == prop_metadata ? void 0 : prop_metadata.parent) !== metadata
          )
    ...
    getOwnPropertyDescriptor(target, prop2) {
      ...
      if (descriptor && "value" in descriptor) {
        ...
        var source2 = sources.get(prop2),
          value2 = null == source2 ? void 0 : source2.v;
        if (void 0 !== source2 && value2 !== UNINITIALIZED)
          return {
            enumerable: true,
            configurable: true,
            value: value2,
    ...
    has(target, prop2) {
      ...
      var s = sources.get(prop2),
        has =
          (void 0 !== s && s.v !== UNINITIALIZED) || Reflect.has(target, prop2);
      if (
        void 0 !== s ||
        (null !== active_effect &&
          (!has ||
    ...
    set(target, prop2, value2, receiver) {
      var _a2,
        s = sources.get(prop2),
        has = prop2 in target;
      if (is_proxied_array && "length" === prop2)
        for (var i = value2; i < s.v; i += 1) {
          var other_s = sources.get(i + "");
          if (void 0 !== other_s) set(other_s, UNINITIALIZED);
          else if (i in target) {
      ...
      if (DEV) {
        var prop_metadata =
          null == value2 ? void 0 : value2[STATE_SYMBOL_METADATA];
        if (
          prop_metadata &&
          (null == prop_metadata ? void 0 : prop_metadata.parent) !== metadata
        )
      ...
      var descriptor = Reflect.getOwnPropertyDescriptor(target, prop2);
      if (null == descriptor ? void 0 : descriptor.set)
    ...
    ownKeys(target) {
      ...
      for (var [key, source2] of sources)
        if (source2.v !== UNINITIALIZED && !(key in target)) own_keys.push(key);
      return own_keys;
    },
    setPrototypeOf() {
      state_prototype_fixed();
...
function get_proxied_value(value) {
  if (null !== value && "object" == typeof value && STATE_SYMBOL in value)
...
function init_array_prototype_warnings() {
  ...
  array_prototype2.lastIndexOf = function (item, from_index) {
    const index2 = lastIndexOf.call(
      this,
      item,
      null != from_index ? from_index : this.length - 1
    );
    if (-1 === index2)
      if (
        -1 !==
...
function child(node, is_text) {
  ...
  if (null === child2) child2 = hydrate_node.appendChild(create_text());
  else if (is_text && 3 !== child2.nodeType) {
...
function validate_effect(rune) {
  ...
  if (null !== active_reaction && !!(active_reaction.f & UNOWNED))
    effect_in_unowned_derived();
...
function push_effect(effect2, parent_effect) {
  ...
  if (null === parent_last) parent_effect.last = parent_effect.first = effect2;
  else {
...
function create_effect(type, fn, sync, push2 = true) {
  var _a,
    is_root = !!(type & ROOT_EFFECT),
    parent_effect = active_effect;
  if (DEV)
    for (; null !== parent_effect && !!(parent_effect.f & INSPECT_EFFECT); )
  ...
  if (
    !(
      ...
      null === effect2.first &&
      null === effect2.nodes_start &&
      null === effect2.teardown &&
      !(effect2.f & EFFECT_HAS_DERIVED)
    ) &&
    !is_root &&
    push2
  ) {
...
function destroy_effect(effect2, remove_dom = true) {
  var removed = false;
  if (
    (remove_dom || !!(effect2.f & HEAD_EFFECT)) &&
    null !== effect2.nodes_start
  ) {
    for (
      var node = effect2.nodes_start, end = effect2.nodes_end;
      null !== node;
...
function pause_children(effect2, transitions, local) {
  if (!(effect2.f & INERT)) {
    effect2.f ^= INERT;
    if (null !== effect2.transitions)
      for (const transition2 of effect2.transitions)
        if (transition2.is_global || local) transitions.push(transition2);
    for (var child2 = effect2.first; null !== child2; ) {
      var sibling2 = child2.next;
      pause_children(
        child2,
        transitions,
        !!(child2.f & EFFECT_TRANSPARENT) || !!(child2.f & BRANCH_EFFECT)
          ? local
          : false
      );
      child2 = sibling2;
...
function resume_children(effect2, local) {
  if (effect2.f & INERT) {
    ...
    if (null !== effect2.transitions)
      for (const transition2 of effect2.transitions)
        if (transition2.is_global || local) transition2.in();
  }
}
...
function handle_event_propagation(event2) {
  var _a,
    ...
    path =
      (null == (_a = event2.composedPath) ? void 0 : _a.call(event2)) || [],
    current_target = path[0] || event2.target,
    path_idx = 0,
    handled_at = event2.__root;
  if (handled_at) {
    ...
    try {
      for (var throw_error, other_errors = []; null !== current_target; ) {
        ...
        try {
          ...
          if (void 0 !== delegated && !current_target.disabled)
...
function template(content, flags) {
  var node,
    is_fragment = !!(flags & TEMPLATE_FRAGMENT),
    use_import_node = !!(flags & TEMPLATE_USE_IMPORT_NODE),
  ...
  return () => {
    ...
    var clone = use_import_node
      ? document.importNode(node, true)
      : node.cloneNode(true);
    if (is_fragment) assign_nodes(get_first_child(clone), clone.lastChild);
...
function hash(str) {
  let hash2 = 5381,
    i = (str = str.replace(regex_return_characters, "")).length;
...
function set_text(text2, value) {
  var _a,
    str = null == value ? "" : "object" == typeof value ? value + "" : value;
...
function hydrate(component2, options) {
  ...
  try {
    for (
      ...
      anchor && (8 !== anchor.nodeType || anchor.data !== HYDRATION_START);
...
function _mount(
  ...
  var component2 = void 0,
    unmount2 = effect_root(() => {
      var anchor_node =
        null != anchor ? anchor : target.appendChild(create_text());
      branch(() => {
        if (context) {
          push({});
      ...
      return () => {
        ...
        if (anchor_node !== anchor)
          null == (_a = anchor_node.parentNode) || _a.removeChild(anchor_node);
      };
    });
...
function unmount(component2) {
  ...
  if (fn) fn();
  else if (DEV) lifecycle_double_unmount();
}
...
function if_block(
  ...
  block(
    () => {
      ...
      if (condition) {
        if (consequent_effect) resume_effect(consequent_effect);
        else consequent_effect = branch(() => consequent_fn(anchor));
        if (alternate_effect)
          pause_effect(alternate_effect, () => {
            alternate_effect = null;
          });
      } else {
        if (alternate_effect) resume_effect(alternate_effect);
...
function pause_effects(state2, items, controlled_anchor, items_map) {
  for (var transitions = [], length = items.length, i = 0; i < length; i++)
    pause_children(items[i].e, transitions, true);
  var is_controlled =
    length > 0 && 0 === transitions.length && null !== controlled_anchor;
  if (is_controlled) {
...
function each(
  ...
  block(() => {
    ...
    let mismatch = false;
    if (hydrating)
      if ((anchor.data === HYDRATION_START_ELSE) != (0 === length)) {
        set_hydrate_node((anchor = remove_nodes()));
        set_hydrating(false);
        mismatch = true;
      }
    ...
    if (null !== fallback_fn)
      if (0 === length)
        if (fallback2) resume_effect(fallback2);
        else fallback2 = branch(() => fallback_fn(anchor));
      else if (null !== fallback2)
...
function reconcile(array, state2, anchor, render_fn, flags, get_key) {
  var _a,
    ...
    item,
    i,
    is_animated = !!(flags & EACH_IS_ANIMATED),
    should_update = !!(flags & (EACH_ITEM_REACTIVE | EACH_INDEX_REACTIVE)),
    length = array.length,
    items = state2.items,
    current = state2.first,
    prev = null,
    matched = [],
    stashed = [];
  if (is_animated)
    ...
    if (void 0 !== (item = items.get(key))) {
      ...
      if (!!(item.e.f & INERT)) {
        ...
        if (is_animated) {
          ...
          (null != to_animate ? to_animate : (to_animate = new Set())).delete(
            item
          );
        }
      }
      ...
      prev = create_item(
        current ? current.e.nodes_start : anchor,
        state2,
        prev,
        null === prev ? state2.first : prev.next,
        value,
        key,
        i,
        render_fn,
  ...
  if (null !== current || void 0 !== seen) {
    for (
      var to_destroy = void 0 === seen ? [] : array_from(seen);
      null !== current;
    ...
    if (destroy_length > 0) {
      var controlled_anchor =
        !!(flags & EACH_IS_CONTROLLED) && 0 === length ? anchor : null;
      if (is_animated) {
        for (i = 0; i < destroy_length; i += 1)
          null == (_c = to_destroy[i].a) || _c.measure();
        for (i = 0; i < destroy_length; i += 1)
          null == (_d = to_destroy[i].a) || _d.fix();
      }
  ...
  if (is_animated)
    queue_micro_task(() => {
      var _a2;
      if (void 0 !== to_animate)
        for (item of to_animate) null == (_a2 = item.a) || _a2.apply();
    });
...
function update_item(item, value, index2, type) {
  ...
  if (!!(type & EACH_INDEX_REACTIVE)) internal_set(item.i, index2);
  else item.i = index2;
}
...
function create_item(
  ...
  try {
    var v = !!(flags & EACH_ITEM_REACTIVE)
        ? !(flags & EACH_ITEM_IMMUTABLE)
          ? mutable_source(value)
          : source(value)
        : value,
      i = !(flags & EACH_INDEX_REACTIVE) ? index2 : source(index2),
      item = {
        i,
        v,
        k: key,
    ...
    if (null === prev) state2.first = item;
    else {
      prev.next = item;
      prev.e.next = item.e;
    }
...
function move(item, next2, anchor) {
  for (
    var end = item.next ? item.next.e.nodes_start : anchor,
      dest = next2 ? next2.e.nodes_start : anchor,
      node = item.e.nodes_start;
    node !== end;
...
function link(state2, prev, next2) {
  if (null === prev) state2.first = next2;
  else {
    prev.next = next2;
...
function check_hash(element2, server_hash, value) {
  var _a, _b;
  if (!server_hash || server_hash === hash(String(null != value ? value : "")))
    return;
  let location;
  const loc = null == (_a = element2.__svelte_meta) ? void 0 : _a.loc;
  if (loc) location = `near ${loc.file}:${loc.line}:${loc.column}`;
  else if (
    null == (_b = dev_current_component_function) ? void 0 : _b[FILENAME]
  )
    location = `in ${dev_current_component_function[FILENAME]}`;
  hydration_html_changed(
    null == location ? void 0 : location.replace(/\//g, "/​")
  );
}
      ...
      if ("" !== value)
        effect2 = branch(() => {
          if (!hydrating) {
            ...
            else anchor.before(node2);
          } else {
            for (
              var hash2 = hydrate_node.data,
                next2 = hydrate_next(),
                last = next2;
              null !== next2 && (8 !== next2.nodeType || "" !== next2.data);
...
function set_attribute(element2, attribute, value, skip_warning) {
  var _a,
    attributes =
      null != (_a = element2.__attributes) ? _a : (element2.__attributes = {});
  if (hydrating) {
    attributes[attribute] = element2.getAttribute(attribute);
    if (
      "src" === attribute ||
      "srcset" === attribute ||
      ("href" === attribute && "LINK" === element2.nodeName)
    ) {
      if (!skip_warning)
        check_src_in_dev_hydration(
          element2,
          attribute,
          null != value ? value : ""
        );
  ...
  if (attributes[attribute] !== (attributes[attribute] = value)) {
    ...
    if (null == value) element2.removeAttribute(attribute);
    else if (
      "string" != typeof value &&
      get_setters(element2).includes(attribute)
    )
      element2[attribute] = value;
    else element2.setAttribute(attribute, value);
  }
}
...
function get_setters(element2) {
  var descriptors,
    setters = setters_cache.get(element2.nodeName);
  if (setters) return setters;
  setters_cache.set(element2.nodeName, (setters = []));
  for (
    var proto = get_prototype_of(element2), element_proto = Element.prototype;
    element_proto !== proto;
...
function check_src_in_dev_hydration(element2, attribute, value) {
  var _a;
  if (DEV)
    if ("srcset" !== attribute || !srcset_url_equal(element2, value))
      if (
        !src_url_equal(
          null != (_a = element2.getAttribute(attribute)) ? _a : "",
          value
        )
      )
        hydration_attribute_changed(
          attribute,
          element2.outerHTML.replace(
            element2.innerHTML,
            element2.innerHTML && "..."
          ),
          String(value)
...
function set_class(dom, value) {
  var prev_class_name = dom.__className,
    next_class_name = to_class(value);
  if (hydrating && dom.className === next_class_name)
    ...
    if (null == value) dom.removeAttribute("class");
    else dom.className = next_class_name;
    dom.__className = next_class_name;
  }
}
...
function bind_value(input, get3, set2 = get3) {
  ...
  listen_to_event_and_reset_event(input, "input", () => {
    ...
    var value = is_numberlike_input(input)
      ? to_number(input.value)
      : input.value;
    set2(value);
  ...
  render_effect(() => {
    ...
    if (!hydrating || input.defaultValue === input.value) {
      if (!is_numberlike_input(input) || value !== to_number(input.value))
        if ("date" !== input.type || value || input.value)
          if (value !== input.value) input.value = null != value ? value : "";
...
function is_bound_this(bound_value, element_or_component) {
  return (
    ...
    (null == bound_value ? void 0 : bound_value[STATE_SYMBOL]) ===
...
function with_parent_branch(fn) {
  for (
    var effect2 = active_effect, previous_effect = active_effect;
    null !== effect2 && !(effect2.f & (BRANCH_EFFECT | ROOT_EFFECT));
...
function prop(props, key, flags, fallback2) {
  var _a,
    prop_value,
    immutable = !!(flags & PROPS_IS_IMMUTABLE),
    runes = !!(flags & PROPS_IS_RUNES),
    bindable = !!(flags & PROPS_IS_BINDABLE),
    lazy = !!(flags & PROPS_IS_LAZY_INITIAL),
    is_store_sub = false;
  if (bindable)
    [prop_value, is_store_sub] = capture_store_binding(() => props[key]);
  else prop_value = props[key];
  var getter,
    setter = null == (_a = get_descriptor(props, key)) ? void 0 : _a.set,
    ...
    get_fallback = () => {
      ...
      if (fallback_dirty) {
        ...
        if (lazy) fallback_value = untrack(fallback2);
        else fallback_value = fallback2;
      }
  ...
  if (runes)
    ...
    var derived_getter = with_parent_branch(() =>
      (immutable ? derived : derived_safe_equal)(() => props[key])
    );
  ...
  var from_child = false,
    ...
    inner_current_value = mutable_source(prop_value),
    current_value = with_parent_branch(() =>
      derived(() => {
        var parent_value = getter(),
          child_value = get(inner_current_value);
        if (
          from_child ||
          (void 0 === parent_value && !!(active_reaction.f & DESTROYED))
        ) {
          from_child = false;
          was_from_child = true;
          return child_value;
  ...
  return function (value, mutation) {
    ...
    if (arguments.length > 0) {
      ...
      if (!current_value.equals(new_value)) {
        ...
        if (fallback_used && void 0 !== fallback_value)
...
var SvelteElement,
  Svelte4Component = class {
    constructor(options) {
      ...
      if (
        !(null == (_b = null == options ? void 0 : options.props)
          ? void 0
          : _b.$$host) ||
        false === options.sync
      )
    ...
    $on(event2, callback) {
      __privateGet(this, _events)[event2] =
        __privateGet(this, _events)[event2] || [];
      const cb = (...args) => callback.call(this, ...args);
      __privateGet(this, _events)[event2].push(cb);
      return () => {
        __privateGet(this, _events)[event2] = __privateGet(this, _events)[
          event2
        ].filter((fn) => fn !== cb);
      };
    }
      ...
      if (!this.$$c) {
        ...
        for (const name of this.$$s)
          if (name in existing_slots)
            if ("default" === name && !this.$$d.children) {
        ...
        this.$$me = effect_root(() => {
          render_effect(() => {
            ...
            for (const key of object_keys(this.$$c)) {
              if (!(null == (_a = this.$$p_d[key]) ? void 0 : _a.reflect))
                continue;
...
function get_custom_element_value(prop2, value, props_definition, transform) {
  ...
  if (!transform || !props_definition[prop2]) return value;
...
var currentFile = writable(""),
  ...
  pluginInstance = writable(void 0);
...
function trimPrefix(source2, prefix) {
  if (source2.startsWith(prefix)) return source2.substring(prefix.length);
  else return source2;
}
...
var import_obsidian = require("obsidian"),
  root = template(
    '<div class="markdownBody svelte-1qfikme" style="min-height: 1em;"></div>'
  ),
  $$css = {
    hash: "svelte-1qfikme",
...
function ScrollViewMarkdownComponent($$anchor, $$props) {
  ...
  user_effect(() => {
    if (
      ...
      file().content &&
      get(el) &&
      get(renderedContent) != file().content
    ) {
...
var on_click = (evt, handleOpenFile, file) => handleOpenFile(evt, get(file)),
  root_1 = template(
    '<div class="file svelte-s1mg0b"><div class="header svelte-s1mg0b"><span> </span> <span class="path svelte-s1mg0b"> </span></div> <!> <hr class="svelte-s1mg0b"></div>'
  ),
  root2 = template(
    '<div class="x"><div class="header svelte-s1mg0b"> </div> <hr class="svelte-s1mg0b"> <!></div>'
  ),
  $$css2 = {
    hash: "svelte-s1mg0b",
...
function ScrollViewComponent($$anchor, $$props) {
  ...
  each(
    ...
    ($$anchor2, file) => {
      ...
      bind_this(
        div_2,
        ($$value) => set(scrollEl, $$value),
        () => get(scrollEl)
      );
      div_2.__click = [on_click, handleOpenFile, file];
      var div_3 = child(div_2),
        span = child(div_3),
        text_1 = child(span, true);
      reset(span);
  ...
  template_effect(() => {
    ...
    return set_text(
      text2,
      `Files with ${null != (_a = get(tagPath)) ? _a : ""}`
    );
  });
  append($$anchor, div);
  pop();
...
function performSortExactFirst(_items, children, leftOverItems) {
  const childrenPathsArr = children
...
function OnDemandRender($$anchor, $$props) {
  ...
  let cssClass = prop($$props, "cssClass", 3, ""),
    isVisible = prop($$props, "isVisible", 15, false),
    hidingScheduled = state(false);
...
var on_click2 = (evt, $$props) =>
    $$props.openFile($$props.item.path, evt.metaKey || evt.ctrlKey),
  on_mouseover = (e, handleMouseover, $$props) => {
    handleMouseover(e, $$props.item.path);
  },
  on_contextmenu = (evt, $$props) =>
    $$props.showMenu(evt, $$props.trail, void 0, [$$props.item]),
  root_2 = template('<div class="tf-taglist"><!></div>'),
  root_12 = template(
    '<div class="tree-item-self is-clickable nav-file-title"><div class="tree-item-inner nav-file-title-content lsl-f"> </div> <!></div>'
  );
...
function V2TreeItemComponent($$anchor, $$props) {
  ...
  const $$stores = setup_stores(),
    $pluginInstance = () =>
      store_get(pluginInstance, "$pluginInstance", $$stores);
  function handleMouseover(e, path) {
    $$props.hoverPreview(e, path);
  }
  const _setting = derived(() =>
      store_get(tagFolderSetting, "$tagFolderSetting", $$stores)
    ),
    _currentActiveFilePath = derived(() =>
      store_get(currentFile, "$currentFile", $$stores)
    );
  let isActive = derived(
      () => $$props.item.path == get(_currentActiveFilePath)
    ),
    isItemVisible = state(false);
  const tagsLeft = derived(() =>
      get(isItemVisible)
        ? uniqueCaseIntensive(
            getExtraTags(
              $$props.item.tags,
              [...$$props.trail],
              get(_setting).reduceNestedParent
            )
    ...
    extraTagsHtml = derived(
      () =>
        `${get(tagsLeft)
          .map((e) => `<span class="tf-tag">${escapeStringToHTML(e)}</span>`)
          .join("")}`
    ),
    draggable = derived(() => !get(_setting).disableDragging),
    app = derived(() =>
      null === $pluginInstance() || void 0 === $pluginInstance()
        ? void 0
        : $pluginInstance().app
    ),
    dm = derived(() =>
      null === get(app) || void 0 === get(app) ? void 0 : get(app).dragManager
    );
  function dragStartFile(args) {
    if (!get(draggable)) return;
    const file = get(app).vault.getAbstractFileByPath($$props.item.path),
      param = get(dm).dragFile(args, file);
...
var on_contextmenu2 = (
    ...
    if (shouldResponsibleFor(evt))
      $$props.showMenu(
        evt,
        [...trail(), ...get(suppressLevels)],
        "tags" == viewType() ? tagName() : get(filename),
        get(_items)
      );
  },
  root_22 = template(
    '<div class="tree-item-self nav-folder-title"><div class="tree-item-inner nav-folder-title-content"> </div></div>'
  ),
  root_6 = template('<svg class="svg-icon"></svg>'),
  root_7 = template('<div class="tagfolder-titletagname"><!></div>'),
  root_8 = template('<div class="tagfolder-titletagname">...</div>'),
  on_click3 = (e, handleOpenScroll, trail, _items) =>
    handleOpenScroll(
      e,
      trail(),
      get(_items).map((e2) => e2.path)
    ),
  root_4 = template(
    '<div class="tree-item-icon collapse-icon nav-folder-collapse-indicator"><!></div> <div class="tree-item-inner nav-folder-title-content lsl-f"><!> <div class="tagfolder-quantity itemscount"><span class="itemscount"> </span></div></div>',
    1
  ),
  root_10 = template("<!> <!>", 1),
  root_15 = template(
  ...
  root4 = template("<div><!> <!></div>");

function V2TreeFolderComponent_1($$anchor, $$props) {
  push($$props, true);
  const $$stores = setup_stores();
  var _a, _b, _c;
  let viewType = prop($$props, "viewType", 3, "tags"),
    thisName = prop($$props, "thisName", 3, ""),
    items = prop($$props, "items", 19, () => []),
    tagName = prop($$props, "tagName", 11, ""),
    tagNameDisp = prop($$props, "tagNameDisp", 27, () => proxy([])),
    trail = prop($$props, "trail", 19, () => []),
    depth = prop($$props, "depth", 3, 1),
    folderIcon = prop($$props, "folderIcon", 3, ""),
    headerTitle = prop($$props, "headerTitle", 3, ""),
    _setting = derived(() =>
      store_get(tagFolderSetting, "$tagFolderSetting", $$stores)
    );
  const expandLimit = derived(() =>
      !get(_setting).expandLimit
        ? 0
        : "links" == viewType()
        ? get(_setting).expandLimit + 1
        : get(_setting).expandLimit
    ),
    _tagInfo = derived(() => store_get(tagInfo, "$tagInfo", $$stores)),
    _currentActiveFilePath = derived(() =>
      store_get(currentFile, "$currentFile", $$stores)
    );
  function handleOpenScroll(e, trails, filePaths) {
    if ("tags" == viewType())
      $$props.openScrollView(
        void 0,
        "",
  ...
  function toggleFolder(evt) {
    ...
    if (shouldResponsibleFor(evt)) {
      ...
      v2expandedTags.update((evt2) => {
        if (evt2.has(get(trailKey))) evt2.delete(get(trailKey));
        else evt2.add(get(trailKey));
        return evt2;
      });
    }
  }
  let _lastParam,
    suppressLevels = state(proxy([])),
    children = state(proxy([])),
    isUpdating = state(false);
  const viewContextID = `${
    null !== (_a = getContext("viewID")) && void 0 !== _a ? _a : ""
  }`;
  ...
  function dragStartFiles(args) {
    if (!get(draggable)) return;
    const files = get(_items).map((e) =>
        get(app).vault.getAbstractFileByPath(e.path)
      ),
      param = get(dm).dragFiles(args, files);
    if (param) return get(dm).onDragStart(args, param);
  }
  function dragStartName(args) {
    ...
    if ("links" == viewType())
      return (function dragStartFile(args) {
        if (!get(draggable)) return;
        const file = get(app).vault.getAbstractFileByPath(get(filename)),
          param = get(dm).dragFile(args, file);
        if (param) return get(dm).onDragStart(args, param);
        else return;
      })(args);
    const expandedTags = [
      ...ancestorToLongestTag(
        ancestorToTags(
          joinPartialPath(
            removeIntermediatePath([...trail(), ...get(suppressLevels)])
          )
        )
      ),
    ]
  ...
  const filename = derived(() =>
    ...
    thisInfo = derived(() =>
      "links" != viewType() ? void 0 : getViewItemFromPath(thisName())
    ),
    thisLinks = derived(() =>
      "links" != viewType()
        ? []
        : (null !==
            (_b =
              null === get(thisInfo) || void 0 === get(thisInfo)
                ? void 0
                : get(thisInfo).links) && void 0 !== _b
            ? _b
            : []
          ).map((e) => `${e}`)
    ),
    thisNameLC = derived(() => thisName().toLowerCase()),
    tagNameLC = derived(() => tagName().toLowerCase()),
    trailKey = derived(() => trail().join("*")),
    trailLower = derived(() => trail().map((e) => e.toLowerCase())),
    collapsed = derived(
      () =>
        !$$props.isRoot &&
        !store_get(v2expandedTags, "$v2expandedTags", $$stores).has(
          get(trailKey)
        )
    ),
    inMiddleOfTagHierarchy = derived(
      () => trail().length >= 1 && trail()[trail().length - 1].endsWith("/")
    ),
    previousTrail = derived(() =>
      get(inMiddleOfTagHierarchy) ? trail()[trail().length - 1] : ""
    ),
    lastTrailTagLC = derived(() =>
      trimTrailingSlash(get(previousTrail)).toLowerCase()
    ),
    _items = derived(items),
    tagsAllCI = derived(() =>
      uniqueCaseIntensive(get(_items).flatMap((e) => e.tags))
    ),
    tagsAllLower = derived(() => get(tagsAllCI).map((e) => e.toLowerCase())),
    isInDedicatedTag = derived(
      () =>
        get(inMiddleOfTagHierarchy) &&
        !get(tagsAllLower).contains(get(lastTrailTagLC))
    ),
    isMixedDedicatedTag = derived(() => get(inMiddleOfTagHierarchy)),
    displayTagCandidates = derived(() => {
      let tagsAll = [];
      if ("links" == viewType()) {
        if (!$$props.isRoot)
          if (!get(_setting).linkShowOnlyFDR)
            tagsAll = get(thisInfo)
              ? getAllLinksRecursive(get(thisInfo), [...trail()])
              : [...get(thisLinks)];
          else tagsAll = [...get(thisLinks)];
        else tagsAll = unique(get(_items).flatMap((e) => e.links));
        if (!$$props.isRoot || get(_setting).expandUntaggedToRoot)
    ...
    tagsExceptAlreadyShown = derived(() =>
      "tags" != viewType()
        ? []
        : get(displayTagCandidates).filter((tag) =>
            trail().every(
              (trail2) =>
    ...
    passedTagWithoutThis = derived(() => {
      const trimSlashedThisNameLC = "/" + trimSlash(thisName()).toLowerCase();
      return get(tagsExceptAlreadyShown)
        .filter((tag) => {
    ...
    escapedPreviousTrail = derived(() =>
      !get(isMixedDedicatedTag)
        ? get(previousTrail)
        : get(previousTrail).split("/").join("*")
    ),
    sparseIntermediateTags = derived(() => {
      const t1 = !get(isInDedicatedTag)
        ? get(passedTagWithoutThis)
        : get(passedTagWithoutThis).filter((e) =>
            (e + "/").startsWith(get(previousTrail))
          );
      if (!get(isInDedicatedTag)) return t1;
      else
        return t1.map((e) =>
          (e + "/").startsWith(get(previousTrail))
            ? get(escapedPreviousTrail) + e.substring(get(previousTrail).length)
            : e
        );
    }),
    tagsPhaseX1 = derived(() => get(sparseIntermediateTags)),
    $$d = derived(() => {
      let isSuppressibleLevel2 = false,
        existTags = get(tagsPhaseX1),
        existTagsFiltered1 = [];
      if (!get(_setting).doNotSimplifyTags && "links" != viewType())
        if (1 == get(_items).length) {
          existTagsFiltered1 = existTags;
          isSuppressibleLevel2 = true;
        } else if (
          1 ==
          uniqueCaseIntensive(
            get(_items).map((e) => [...e.tags].sort().join("**"))
          ).length
        ) {
          isSuppressibleLevel2 = true;
          existTagsFiltered1 = existTags;
      ...
      if (!isSuppressibleLevel2) {
        ...
        if (get(_setting).reduceNestedParent)
          removeItems.push(...get(trailLower));
        let tagsOnNextLevel = [];
        if ("tags" == viewType())
          tagsOnNextLevel = uniqueCaseIntensive(
            existTags.map((e) => {
              ...
              for (
                ;
                removeItems.some((e2) => e2.startsWith(piece.toLowerCase()));
      ...
      if (get(isMixedDedicatedTag) || get(isInDedicatedTag))
        existTagsFiltered1 = existTagsFiltered1.map((e) =>
          e.replace(get(escapedPreviousTrail), get(previousTrail))
        );
      if (get(isMixedDedicatedTag) || get(isInDedicatedTag))
        existTagsFiltered1 = existTagsFiltered1.map((e) =>
          e.replace(get(escapedPreviousTrail), get(previousTrail))
      ...
      return {
        filteredTags: uniqueCaseIntensive(
    ...
    filteredTags = derived(() => get($$d).filteredTags),
    isSuppressibleLevel = derived(() => get($$d).isSuppressibleLevel),
    $$d_1 = derived(() => {
      let tags2 = [];
      ...
      if (
        !(
          $$props.isMainTree &&
          (!get(expandLimit) ||
            (get(expandLimit) && depth() < get(expandLimit)))
        )
      )
        return {
          tags: tags2,
      ...
      if (get(previousTrail).endsWith("/")) {
        ...
        for (const tag of get(filteredTags))
          if (
            !get(filteredTags)
              .map((e) => e.toLowerCase())
              .contains((get(previousTrail) + tag).toLowerCase())
          )
            existTagsFiltered4.push(tag);
        tags2 = uniqueCaseIntensive(removeIntermediatePath(existTagsFiltered4));
      } else
        tags2 = uniqueCaseIntensive(removeIntermediatePath(get(filteredTags)));
      return {
        tags: tags2,
        leftOverItemsSrc: leftOverItemsSrc2,
      };
    }),
    tags = derived(() => get($$d_1).tags),
    leftOverItemsSrc = derived(() => get($$d_1).leftOverItemsSrc),
    linkedItems = derived(() => {
      const ret = new Map();
      if ("tags" == viewType()) return ret;
      for (const tag of get(displayTagCandidates))
        if ("_unlinked" == tag)
          ret.set(
            tag,
            get(_items).filter((e) => e.links.contains(tag))
          );
        else {
    ...
    tagsOfLinkedItems = derived(() => {
      ...
      if ("_unlinked" == thisName()) leftOverItems2 = get(_items);
      else
        get(displayTagCandidates).forEach((tag) => {
          if ("_unlinked" == tag) {
            tags2.push(tag);
            return;
          }
          ...
          const existLinks = x.links.filter(
            ...
            nextDepth =
              !get(expandLimit) ||
              (get(expandLimit) && depth() + 1 < get(expandLimit));
          if (existLinks.length >= 2 && nextDepth) tags2.push(tag);
          else leftOverItems2.push(x);
        });
      return {
        tags: tags2,
        leftOverItems: leftOverItems2,
    ...
    leftOverItemsUnsorted = derived(() => {
      ...
      if ($$props.isRoot && $$props.isMainTree && !get(isSuppressibleLevel))
        if (get(_setting).expandUntaggedToRoot)
          return get(_items).filter(
            (e) => e.tags.contains("_untagged") || e.tags.contains("_unlinked")
          );
        else return [];
      if ($$props.isRoot && !$$props.isMainTree) return get(_items);
      if ("tags" == viewType())
        if ("NONE" == get(_setting).hideItems) return get(_items);
        else if (
          ("DEDICATED_INTERMIDIATES" == get(_setting).hideItems &&
            get(isInDedicatedTag)) ||
          "ALL_EXCEPT_BOTTOM" == get(_setting).hideItems
        )
          return get(_items).filter(
            (e) =>
              !get(children)
        ...
        else return get(_items);
      else return get(leftOverItemsSrc);
    }),
    leftOverItems = derived(() =>
      get(_setting).sortExactFirst
        ? performSortExactFirst(
            get(_items),
            get(children),
            get(leftOverItemsUnsorted)
          )
        : get(leftOverItemsUnsorted)
    );
  let isActive = derived(
    () =>
      (get(_items) &&
        get(_items).some((e) => e.path == get(_currentActiveFilePath))) ||
      ("links" == viewType() &&
        (thisName() == get(_currentActiveFilePath) ||
          get(tags).contains(get(_currentActiveFilePath)) ||
          get(leftOverItems).some(
            (e) => e.path == get(_currentActiveFilePath)
          )))
  );
  const tagsDisp = derived(() =>
      get(isSuppressibleLevel) && get(isInDedicatedTag)
        ? [
            [
              ...tagNameDisp(),
              ...get(suppressLevels).flatMap((e) =>
                e.split("/").map((e2) => renderSpecialTag(e2))
              ),
            ],
          ]
        : get(isSuppressibleLevel)
        ? [
            tagNameDisp(),
            ...get(suppressLevels).map((e) =>
              e.split("/").map((e2) => renderSpecialTag(e2))
            ),
          ]
        : [tagNameDisp()]
    ),
    classKey = derived(() => ("links" == viewType() ? " tf-link" : " tf-tag")),
    tagsDispHtml = derived(() =>
      get(isFolderVisible)
        ? get(tagsDisp)
            .map(
              (e) =>
                `<span class="tagfolder-tag tag-tag${get(classKey)}">${e
                  .map(
                    (ee) =>
    ...
    itemCount = derived(() =>
      "tags" == viewType()
        ? null !==
            (_c =
              null === get(_items) || void 0 === get(_items)
                ? void 0
                : get(_items).length) && void 0 !== _c
          ? _c
          : 0
        : get(tags).length + get(leftOverItems).length
    ),
    leftOverItemsDisp = derived(() => splitArrayToBatch(get(leftOverItems))),
    childrenDisp = derived(() => splitArrayToBatch(get(children))),
    draggable = derived(() => !get(_setting).disableDragging),
    app = derived(
      () => store_get(pluginInstance, "$pluginInstance", $$stores).app
    ),
    dm = derived(() =>
      null === get(app) || void 0 === get(app) ? void 0 : get(app).dragManager
    );
  user_effect(() => {
    const key = get(trailKey) + ($$props.isRoot ? "-r" : "-x") + viewContextID,
      sortFunc = selectCompareMethodTags(
        get(_setting),
        "links" == viewType() ? {} : get(_tagInfo)
      );
    (function updateX(param) {
      if (!isSameAny(param, _lastParam)) {
        ...
        if (param.isFolderVisible || $$props.isRoot)
  ...
  if_block(
    ...
    ($$anchor2) => {
      var cssClass = derived(
        () =>
          `tree-item-self${
            !$$props.isRoot ? " is-clickable mod-collapsible" : ""
        ...
        children: ($$anchor3, $$slotProps) => {
          ...
          if_block(
            ...
            ($$anchor4) => {
              ...
              reset(div_5);
              template_effect(() =>
                set_attribute(div_5, "draggable", get(draggable))
              );
          ...
          each(
            ...
            ($$anchor5, $$item) => {
              ...
              var trail_1 = derived(() => [
                ...
                depth_1 = derived(() =>
                  get(isInDedicatedTag) ? depth() : depth() + 1
                );
  ...
  template_effect(() =>
    set_class(
      div,
      `tree-item nav-folder${get(collapsed) ? " is-collapsed" : ""}${
        $$props.isRoot ? " mod-root" : ""
      }${get(isUpdating) ? " updating" : ""}`
    )
  );
  append($$anchor, div);
  pop();
...
var root_13 = template(
    '<div class="clickable-icon nav-action-button" aria-label="Change sort order"><!></div>  <div class="clickable-icon nav-action-button" aria-label="Expand limit"><!></div>  <div aria-label="Search"><!></div>',
    1
  ),
  root_23 = template(
    '<div class="clickable-icon nav-action-button" aria-label="Switch List/Tree"><!></div>'
  ),
  root_3 = template(
    '<div class="clickable-icon nav-action-button" aria-label="Toggle Incoming"><!></div>  <div class="clickable-icon nav-action-button" aria-label="Toggle Outgoing"><!></div>  <div class="clickable-icon nav-action-button" aria-label="Toggle Incoming&amp;Outgoing"><!></div>  <div class="clickable-icon nav-action-button" aria-label="Toggle Hide indirect notes"><!></div>',
    1
  ),
  root_42 = template(
    '<div class="search-row"><div class="search-input-container global-search-input-container"><input type="search" spellcheck="false" placeholder="Type to start search...">  <div class="search-input-clear-button" aria-label="Clear search"></div></div></div>'
  ),
  root5 = template(
    '<div hidden></div> <div class="nav-header"><div class="nav-buttons-container tagfolder-buttons-container"><div class="clickable-icon nav-action-button" aria-label="New note"><!></div> <!> <!> <!></div></div> <!> <div class="nav-files-container node-insert-event svelte-1xm87ro"><!></div>',
    1
  ),
  $$css3 = {
    hash: "svelte-1xm87ro",
...
function TagFolderViewComponent($$anchor, $$props) {
  ...
  const $$stores = setup_stores(),
    $searchString = () => store_get(searchString, "$searchString", $$stores);
  let vaultName = prop($$props, "vaultName", 3, ""),
    title = prop($$props, "title", 15, ""),
    tags = prop($$props, "tags", 31, () => proxy([])),
    viewType = prop($$props, "viewType", 3, "tags");
  const isMainTree = derived(() => 0 == tags().length);
  null === $$props.stateStore ||
    void 0 === $$props.stateStore ||
    $$props.stateStore.subscribe((state2) => {
      tags(state2.tags);
      title(state2.title);
    });
  ...
  const viewItemsSrc = derived(() => {
    if ("tags" == viewType())
      return store_get(allViewItems, "$allViewItems", $$stores);
    else return store_get(allViewItemsByLink, "$allViewItemsByLink", $$stores);
  });
  let _setting = state(
      proxy(store_get(tagFolderSetting, "$tagFolderSetting", $$stores))
    ),
    outgoingEnabled = state(false),
    incomingEnabled = state(false),
    bothEnabled = state(false),
    onlyFDREnabled = state(false);
    ...
    const incoming =
        null !==
          (_c =
            null ===
              (_b =
                null === (_a = get(_setting).linkConfig) || void 0 === _a
                  ? void 0
                  : _a.incoming) || void 0 === _b
              ? void 0
      ...
      outgoing =
        null !==
          (_f =
            null ===
              (_e =
                null === (_d = get(_setting).linkConfig) || void 0 === _d
                  ? void 0
                  : _d.outgoing) || void 0 === _e
              ? void 0
  ...
  let observer,
    ...
    iconDivEl = state(void 0),
    newNoteIcon = state(""),
    folderIcon = state(""),
    upAndDownArrowsIcon = state(""),
    stackedLevels = state(""),
    searchIcon = state(""),
    switchIcon = state(""),
    outgoingIcon = state(""),
  ...
  setContext("observer", {
    observe: function observe(el, callback) {
      if (!observer) observingElQueue.push(el);
      else if (observingElQueue.length > 0) observeAllQueued();
  ...
  let headerTitle = derived(() =>
    "" == title()
      ? `${"tags" == viewType() ? "タグ" : "Links"}: ${vaultName()}`
      : `Items: ${title()}`
  );
  const viewItems = derived(() => {
    ...
    const firstLevel = trimTrailingSlash(
        null !== (_a = tags().first()) && void 0 !== _a ? _a : ""
      ).toLowerCase(),
      archiveTags = get(_setting)
        .archiveTags.toLowerCase()
        .replace(/[\n ]/g, "")
        .split(",");
    if (!archiveTags.contains(firstLevel))
      items = items.filter(
        (item) => !item.tags.some((e) => archiveTags.contains(e.toLowerCase()))
      );
    return items;
  });
  ...
  if_block(
    ...
    ($$anchor2) => {
      ...
      const style_derived = derived(() => {
        var _a;
        return `display:${
          null != (_a = "" == $searchString().trim() ? "none" : "") ? _a : ""
        };`;
      });
      ...
      bind_value(input, $searchString, ($$value) =>
        store_set(searchString, $$value)
      );
...
var import_obsidian5 = require("obsidian"),
  ...
  askString = (app, title, placeholder, initialText) =>
    new Promise((res) => {
      new PopoverSelectString(app, title, placeholder, initialText, (result) =>
        res(result)
      ).open();
    }),
...
var TagFolderViewBase = class extends import_obsidian5.ItemView {
  ...
  TagFolderView = class extends TagFolderViewBase {
    ...
    getDisplayText() {
      return "tags" == this.treeViewType ? "Tag Folder" : "リンクフォルダ";
    }
    async onOpen() {
  ...
  HideItemsType = {
    NONE: "何も隠さない",
    DEDICATED_INTERMIDIATES: "ネストされたタグの中間のみ",
    ALL_EXCEPT_BOTTOM: "すべての中間",
  };
...
var TagFolderPlugin5 = class extends import_obsidian8.Plugin {
            ...
            if (null != this.getLinkView()) {
              const oldLinks = (null == old ? void 0 : old.links) || [],
                ...
                diffs2 = unique([...oldLinks, ...newLinks]).filter(
                  (link2) =>
                    !oldLinks.contains(link2) || !newLinks.contains(link2)
                );
    ...
    async openListView(tagSrc) {
      ...
      for (const leaf of this.app.workspace.getLeavesOfType(
        ...
        if (null == (_a = state2.state) ? void 0 : _a.tags) {
          if (
            state2.state.tags.slice().sort().join("-") ==
      ...
      if (!theLeaf) {
        const parent =
          null == (_b = this.app.workspace.getLeavesOfType(VIEW_TYPE_TAGFOLDER))
            ? void 0
            : _b.first();
        if (!parent) return;
  ...
  TagFolderSettingTab = class extends import_obsidian8.PluginSettingTab {
    ...
    display() {
      ...
      new import_obsidian8.Setting(containerEl)
        .setName("タグをクリックした時にTagFolder内でタグを検索")
        .addToggle((toggle) => {
          toggle
      ...
      new import_obsidian8.Setting(containerEl)
        .setName("リストの表示場所")
        .setDesc("このオプションは新しく開かれたリストに適用されます")
        .addDropdown((dropdown) => {
          dropdown
      ...
      new import_obsidian8.Setting(containerEl)
        .setName("アイテムを隠す")
        .setDesc("ランディングページまたはネストされたタグでアイテムを隠す")
        .addDropdown((dd) => {
      ...
      new import_obsidian8.Setting(containerEl)
        .setName("空のフォルダを簡略化しない")
        .setDesc("簡略化できる場合でも、空のフォルダを保持します。")
        .addToggle((toggle) => {
          toggle
      ...
      new import_obsidian8.Setting(containerEl)
        .setName("ネストされたタグを専用レベルとして扱わない")
        .setDesc("ネストされたタグを通常のタグとして扱う")
        .addToggle((toggle) => {
          toggle
      ...
      new import_obsidian8.Setting(containerEl)
        .setName("ネストされたタグの重複した親を減らす")
        .setDesc(
          "有効にすると、#web/css、#web/javascript は web -> css -> javascript にマージされます"
        )
        .addToggle((toggle) => {
          toggle
      ...
      new import_obsidian8.Setting(containerEl)
        .setName("対象フォルダ")
        .setDesc("プラグインはその中のファイルのみを対象とします。")
        .addTextArea((text2) =>
          text2
      ...
      new import_obsidian8.Setting(containerEl)
        .setName("無視するタグ")
        .setDesc("リスト内のタグは存在しないものとして扱われます。")
        .addTextArea((text2) =>
          text2
      ...
      new import_obsidian8.Setting(containerEl)
        ...
        .addButton((button) =>
          button
            .setButtonText("偽装したタグをコピー")
            .setDisabled(false)
            .onClick(async () => {
              const x = new Map();
              let i = 0;
              const items = (await this.plugin.getItemsList("tag")).map((e) =>
                e.tags
