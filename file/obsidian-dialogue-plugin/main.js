...
(function (DialogueTitleMode2) {
  DialogueTitleMode2["Disabled"] = "無効";
  DialogueTitleMode2["First"] = "最初のみ";
  DialogueTitleMode2["All"] = "すべて";
...
var Message = class {
  constructor(content, side, dialogueSettings) {
    ...
    this.participant =
      this.side == SIDES.LEFT
        ? this.dialogueSettings.leftParticipant
        : this.dialogueSettings.rightParticipant;
    this.renderMessage();
  }
  renderMessage() {
    ...
    if (this.titleShouldRender()) {
      messageEl.createDiv({
        cls: CLASSES.MESSAGE_TITLE,
        text: this.participant.title,
      });
    }
  ...
  createMessageEl() {
    ...
    const sideClass =
      this.side == SIDES.LEFT
        ? CLASSES.MESSAGE_WRAPPER_LEFT
        : CLASSES.MESSAGE_WRAPPER_RIGHT;
    ...
    return messageWrapperEl.createDiv({
      ...
      attr: {
        ...
        "data-participant-id":
          (_a = this.participant.enforcedId) != null
            ? _a
            : this.dialogueSettings.participants.get(this.participant.title),
      },
    });
  }
  titleShouldRender() {
...
var Delimiter = class {
  ...
  renderDelimiter() {
    ...
    const delimiterEl = delimiterWrapperEl.createDiv({
      cls: CLASSES.DELIMITER,
    });
...
var DialogueRenderer = class {
  constructor(src, parent, settings) {
    this.src = src;
    this.dialogueWrapperEl = parent.createDiv({
      cls: CLASSES.DIALOGUE_WRAPPER,
    });
  ...
  registerParticipant(participant) {
    if (!this.dialogueSettings.participants.has(participant)) {
      this.dialogueSettings.participants.set(
        participant,
        this.dialogueSettings.participants.size + 1
      );
    }
  }
  ...
  renderDialogue() {
    ...
    for (const line of lines) {
      if (KEYWORDS.LEFT_PATTERN.test(line)) {
        this.dialogueSettings.leftParticipant.title = line
          .split(":")
          .splice(1)
          .join(":")
          .trim();
        this.dialogueSettings.leftParticipant.renderedOnce = false;
        this.dialogueSettings.leftParticipant.enforcedId = this.getEnforcedId(
          KEYWORDS.LEFT_PATTERN,
          line
        );
      } else if (KEYWORDS.RIGHT_PATTERN.test(line)) {
        this.dialogueSettings.rightParticipant.title = line
          .split(":")
          .splice(1)
          .join(":")
          .trim();
        this.dialogueSettings.rightParticipant.renderedOnce = false;
        this.dialogueSettings.rightParticipant.enforcedId = this.getEnforcedId(
          KEYWORDS.RIGHT_PATTERN,
          line
        );
      } else if (line.startsWith(KEYWORDS.TITLE_MODE)) {
        const modeName = line
          .substr(KEYWORDS.TITLE_MODE.length)
          .trim()
          .toLowerCase();
        ...
        this.dialogueSettings.messageMaxWidth = line
          .substr(KEYWORDS.MESSAGE_MAX_WIDTH.length)
          .trim();
      } else if (line.startsWith(KEYWORDS.COMMENT_MAX_WIDTH)) {
        this.dialogueSettings.commentMaxWidth = line
          .substr(KEYWORDS.COMMENT_MAX_WIDTH.length)
          .trim();
...
var DialogueSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    containerEl.createEl("h2", { text: "ダイアログ設定" });
    const coffeeEl = containerEl.createEl("div", {
      attr: {
        style: "text-align: center; margin-bottom: 10px;",
      },
    });
    const coffeeLinkEl = coffeeEl.createEl("a", {
      href: "https://www.buymeacoffee.com/holubj",
    });
    ...
    new import_obsidian.Setting(containerEl)
      .setName("デフォルト左タイトル")
      .setDesc("すべてのダイアログで使用する左タイトルのデフォルト値")
      .addText((text) =>
        text
          .setPlaceholder("デフォルトの左タイトルを入力")
    ...
    new import_obsidian.Setting(containerEl)
      .setName("デフォルト右タイトル")
      .setDesc("すべてのダイアログで使用する右タイトルのデフォルト値")
      .addText((text) =>
        text
          .setPlaceholder("デフォルトの右タイトルを入力")
    ...
    new import_obsidian.Setting(containerEl)
      .setName("デフォルトタイトルモード")
      .setDesc("すべてのダイアログで使用するデフォルトのタイトルモード")
      .addDropdown((cb) => {
        ...
        cb.setValue(this.plugin.settings.defaultTitleMode).onChange((value) =>
          __async(this, null, function* () {
    ...
    new import_obsidian.Setting(containerEl)
      .setName("デフォルト最大メッセージ幅")
      .setDesc("すべてのダイアログで使用する最大メッセージ幅のデフォルト値")
      .addText((text) =>
        text
          .setPlaceholder("デフォルト最大メッセージ幅を入力")
    ...
    new import_obsidian.Setting(containerEl)
      .setName("デフォルト最大コメント幅")
      .setDesc("Default max comment width in all dialogues.")
      .addText((text) =>
        text
          .setPlaceholder("デフォルト最大コメント幅を入力")
