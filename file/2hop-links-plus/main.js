...
var TwohopSettingTab = class extends import_obsidian7.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    this.createToggleSetting("２ホップリンクを別のペインに表示", "true の場合、２ホップリンクは別のペインに表示される。", "showTwoHopLinksInSeparatePane");
    if (this.plugin.settings.showTwoHopLinksInSeparatePane) {
      this.createToggleSetting("右側に２ホップリンクを表示", "true の場合、２ホップリンク用のペインが右に表示され、そうでない場合は左に表示される。", "panePositionIsRight");
    }
    this.createDropdownSetting("ソート順", "", "sortOrder", {
      random: "ランダム",
      filenameAsc: "名前 (A-Z)",
      filenameDesc: "名前 (Z-A)",
    ...
    this.createToggleSetting("リンクの表示", "", "showForwardConnectedLinks");
    this.createToggleSetting("バックリンクの表示", "", "showBackwardConnectedLinks");
    this.createToggleSetting("２ホップリンクの表示", "", "showTwohopLinks");
    this.createToggleSetting("新規リンクの表示", "", "showNewLinks");
    this.createToggleSetting("タグリンクの表示", "", "showTagsLinks");
    this.createToggleSetting("プロパティリンクの表示", "", "showPropertiesLinks");
    this.createToggleSetting("２ホップリンク内に画像を表示", "", "showImage");
    this.createTextAreaSetting("除外するパス", "除外するファイル、フォルダのパス（１行毎に１つ）", "excludePaths", "path/to/file.md\npath/to/folder/");
    this.createTextAreaSetting("除外するタグ", "除外するタグ（１行毎に１つ）", "excludeTags", "tagNameToExclude\nparent/childTagToExclude\nparentTag/forAllSubtags/");
    this.createTextAreaSetting("フロントマターキー", "含めるフロントマターのキー（これらのキーの値はタグのように扱われる）", "frontmatterKeys", "key1\nkey2\nkey3");
    this.createTextSettingNum("初期ボックス数", "表示するボックスの初期数", "initialBoxCount");
    this.createTextSettingNum("初期セクション数", "表示するセクションの初期数", "initialSectionCount");
    this.createToggleSetting("重複削除の有効化", "重複リンクを削除", "enableDuplicateRemoval");
    this.createToggleSetting("２ホップリンクの自動読み込み", "ノート表示時に２ホップリンクを自動的に読み込む", "autoLoadTwoHopLinks");
    this.createToggleSetting("複数リンク用ファイルの作成", "複数ファイルに接続されているリンクに新規ファイルを作成", "createFilesForMultiLinked");
    this.createTextSettingStr("フロントマターのプロパティ・キーをタイトルに設定", "表示タイトルに使用するフロントマターのプロパティ・キーの設定", "frontmatterPropertyKeyAsTitle");
  }
