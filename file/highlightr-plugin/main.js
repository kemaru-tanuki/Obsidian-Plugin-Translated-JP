...
class HighlightrSettingTab extends obsidian.PluginSettingTab {
    ...
    display() {
        ...
        containerEl.createEl("h2", { text: "プラグイン設定" });
        new obsidian.Setting(containerEl)
            .setName("ハイライト方法を選択")
            .setDesc(`インラインCSSまたはCSSクラスでのハイライトを選択してください。両方の選択肢にはそれぞれ長所と短所があることに注意してください。インラインCSSを使用すると、ノートをエクスポートする際に外部CSSファイルに依存しなくて済みます。CSSクラスはより柔軟でカスタマイズが容易です。`)
        ...
        stylesSetting
            .setName("ハイライトスタイルを選択")
            .setDesc(`デザインの美学に応じて、ハイライトのスタイルをカスタマイズしたい場合があります。ドロップダウンを使用して、さまざまなハイライタースタイルから選択してください。テーマによっては、このプラグインのCSSが上書きされる可能性があります。`)
            .addDropdown((dropdown) => {
            let styles = {};
      ...
      <span style="background:#CCA9FFCC;margin: 0 -0.05em;padding: 0.125em 0.15em;border-radius: 0.2em;-webkit-box-decoration-break: clone;box-decoration-break: clone;">Rounded</span>`;
        ...
        highlighterSetting
            ...
            .setDesc(`色名を指定し、カラーピッカーを使用してHEXコード値を設定することで、新しいハイライトカラーを作成します。カラーピッカーを終了する前に、色を保存することを忘れないでください。ハイライトカラーをドラッグ＆ドロップして、ハイライターコンポーネントの順序を変更できます。`);
