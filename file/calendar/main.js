...
class CalendarSettingsTab extends obsidian.PluginSettingTab {
    ...
    display() {
        ...
        if (this.plugin.options.showWeeklyNote &&
            ...
            this.containerEl.createEl("p", {
                cls: "setting-item-description",
                text: "注意：週間ノート設定は移動します。今後の機能維持のため、「Periodic Notes」プラグインをインストールすることを推奨します。",
            });
    ...
    addWeekStartSetting() {
        ...
        new obsidian.Setting(this.containerEl)
            .setName("週の開始日:")
            .setDesc("週の開始曜日を選択します。「ロケールデフォルト」を選択すると、moment.jsで指定されたデフォルトが使用されます")
    ...
    addConfirmCreateSetting() {
        new obsidian.Setting(this.containerEl)
            .setName("新規ノート作成前に確認")
            .setDesc("新規ノートを作成する前に確認モーダルを表示します")
            .addToggle((toggle) => {
    ...
    addShowWeeklyNoteSetting() {
        new obsidian.Setting(this.containerEl)
            .setName("週番号を表示")
            .setDesc("これを有効にすると、週番号の列が追加されます")
            .addToggle((toggle) => {
    ...
    addWeeklyNoteFormatSetting() {
        new obsidian.Setting(this.containerEl)
            .setName("週間ノートの形式")
            .setDesc("構文の詳細については、形式リファレンスを参照してください")
            .addText((textfield) => {
    ...
    addWeeklyNoteTemplateSetting() {
        new obsidian.Setting(this.containerEl)
            .setName("週間ノートのテンプレート")
            .setDesc("週間ノートのテンプレートとして使用するファイルを選択します")
            .addText((textfield) => {
    ...
    addLocaleOverrideSetting() {
        ...
        new obsidian.Setting(this.containerEl)
            .setName("ロケールを上書き:")
            .setDesc("デフォルトと異なるロケールを使用する場合に設定します")
            .addDropdown((dropdown) => {
