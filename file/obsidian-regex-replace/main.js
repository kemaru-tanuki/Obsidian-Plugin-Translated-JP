...
class RegexFindReplaceSettingTab extends obsidian.PluginSettingTab {
    ...
    display() {
        ...
        containerEl.empty();
        containerEl.createEl('h4', { text: '正規表現の設定' });
        new obsidian.Setting(containerEl)
            .setName('大文字小文字を区別しない')
            .setDesc('正規表現を使用する際、大文字小文字を区別しない検索のために「i」修飾子を適用する。')
        ...
        containerEl.createEl('h4', { text: '一般設定' });
        new obsidian.Setting(containerEl)
            .setName('\\n を改行として処理')
        ...
        new obsidian.Setting(containerEl)
            .setName('検索フィールドを事前に入力')
            .setDesc('現在選択中のテキスト（ある場合）を「検索テキストフィールド」にコピーします（選択範囲に改行が含まない場合のみ適用）。')
            .addToggle(toggle => toggle
