...
var LoaderSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian.Setting(containerEl).setName(".txtファイルを読み込む").addToggle((toggle) => toggle.setValue(this.plugin.settings.doLoadTxt).onChange(async (value) => {
      this.plugin.settings.doLoadTxt = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName(".txtファイルを作成する").addToggle((toggle) => toggle.setValue(this.plugin.settings.doCreateTxt).onChange(async (value) => {
      this.plugin.settings.doCreateTxt = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName(".jsonファイルを読み込む").addToggle((toggle) => toggle.setValue(this.plugin.settings.doLoadJson).onChange(async (value) => {
      this.plugin.settings.doLoadJson = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName(".jsonファイルを作成する").addToggle((toggle) => toggle.setValue(this.plugin.settings.doCreateJson).onChange(async (value) => {
      this.plugin.settings.doCreateJson = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName(".xmlファイルを読み込む").addToggle((toggle) => toggle.setValue(this.plugin.settings.doLoadXml).onChange(async (value) => {
      this.plugin.settings.doLoadXml = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName(".xmlファイルを作成する").addToggle((toggle) => toggle.setValue(this.plugin.settings.doCreateXml).onChange(async (value) => {
